...
def flip_bits(number, bit_length=64, start_bit=0, end_bit=63):
    ...
    # print(number)
    return number / (2**(bit_length-1))

def stuck_bits(number, bit_length=64, start_bit=0, end_bit=63, stuck='1'):
    number = int(float(number) * (2**(bit_length-1)))
    binary_tmp = str(np.binary_repr(number, width=bit_length))
    # print(binary_tmp)
    position = random.randint(start_bit, end_bit)
    binary_tmp = binary_tmp[:position] + stuck + binary_tmp[position+1:]
    # print(binary_tmp)
    number = int(TwosComplement.to_float(binary_tmp))
    # print(number)
    return number / (2**(bit_length-1))
...
def insert_faults(modified_data, ftype, frate):
    ...
    while (i < n_faults):
        ...
        # try:
        if ftype == 'power-gate':
            modified_data[findexes] = 0.0
        elif ftype == 'stuck-at-zero':
            modified_data[findexes] = stuck_bits(modified_data[findexes].to(torch.float32), stuck='0')
        elif ftype == 'stuck-at-one':
            modified_data[findexes] = stuck_bits(modified_data[findexes].to(torch.float32), stuck='1')
        elif ftype == 'flip-bit':
            modified_data[findexes] = flip_bits(modified_data[findexes].to(torch.float32))
        # except:
        #     print(rand_indexes)
        #     print(findexes)
        # print(state['state_dict'][key].data[findexes])
...
def evaluate(model, criterion, data_loader, device, print_freq=100, header='Test:', ftype='flip-bit', frate=0.001, insert_faults=False):
    if (insert_faults==True):
        ### Uncomment for inserting faults to a single layer only.
        # new_model = insert_fault_to_model(model, layer="layer2", ftype=ftype, frate=frate, all_layer=False)
        model = insert_fault_to_model(model, ftype=ftype, frate=frate)
...
def main(args):
    ...
    if args.test_only:
        evaluate(model, criterion, data_loader_test, device=device, header='Test:', ftype=args.ftype, frate=args.frate, insert_faults=args.is_fault)
        return
...
def parse_args():
    ...
    parser.add_argument('--start-epoch', default=0, type=int, metavar='N',
                        help='start epoch')
    parser.add_argument('--ftype', default='flip-bit', type=str, help='fault type: stuck-at-zero, stuck-at-one, flip-bit, power-gate')
    parser.add_argument('--frate', default=0.001, type=float, help='fault rate')
    parser.add_argument('--is-fault', default=False, type=bool, help='fault rate', dest='is_fault')
    parser.add_argument(
        "--cache-dataset",
        dest="cache_dataset",
